...
const vcard = 'BEGIN:VCARD\n' // JAN DI UBAH
            ...
            + 'FN:OWNER GANS\n' // NAMA KONTAK
            + 'ORG:ANKER;\n' // CREATOR
            + 'TEL;type=CELL;type=VOICE;waid=559985000278:+55 99 8500-0278\n' // NOMER LU
            + 'END:VCARD'
prefix = '#'
blocked = ['559985000278@s.whatsapp.net']
		...
		try {
			...
			mess = {
				...
				only: {
					...
					public: '[❗] Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Dark Bot, ketik #darkgroup\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*! ',
					premium: '[❗] PERINTAH INI KHUSUS USER *PREMIUM*',
					benned: 'Anda terkena Band Silahkan Hub Owner wa.me/559985000278',
					ownerG: '[❗] Perintah ini hanya bisa di gunakan oleh owner group! ❌',
					ownerB: '[❗] Perintah ini hanya bisa di gunakan oleh owner bot! ❌',
					userB: `*Upss Kamu Belum Terdaftar Didatabase BOT Dark Bot*\n──「 DAFTAR 」──\nHalo kak !\nKamu belum Terdaftar didalam database, \n\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar ${pushname2}|17\n\n──「 DARK BOT 」──`,
...
const koin = ['http://bit.ly/Koin1','http://bit.ly/Koin5','https://i.ibb.co/q7kdBjm/be27e6f849da.jpg','https://i.ibb.co/BCxNPD5/a42ef753a321.jpg']
			...
			const ownerNumber = ["559985000278@s.whatsapp.net","559985000278@s.whatsapp.net"] // replace this with your number
			const adminbotnumber = ["6281368646011@s.whatsapp.net"]
			const frendsowner = ["6281368646011@s.whatsapp.net"]
			const vip = ["559985000278@s.whatsapp.net","6281368646011@s.whatsapp.net"]
...
case 'burnpaper':
					...
					break
                   case 'sep':
                   if (!isOwner) return replay(`khusus Owner Tolol`)
                   if (!isPremium) return reply(mess.only.premium)
                    if (!isUser) return reply(mess.only.userB)
                   if (isBanned) return reply(mess.only.benned)
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam ${pushname2}:)`)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('6283806369677@s.whatsapp.net', options, text, {quoted: mek})
...
client.updatePresence(from, Presence.composing) 
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions(`${sew}\n\nTagall by~~ *${pushname2}*\n\n╔══✪〘 Mention All 〙✪══\n╠➥ *Pesan:* `+teks+`╚═〘 DARK BOT 〙`, members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
	...
	.then(() => {client.sendMessage(from, options, text)})
               ...
               client.updatePresence(from, Presence.composing) 
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`Ini adalah list group DARK BOT :\n\n\`\`\``
					no = 0
				...
				//frendowner
				case 'addadmin':
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner DARK BOT!', id)
                   i = body.slice(8)
                   adminbotnumber.push(i)
                   fs.writeFileSync('./src/admin.json', JSON.stringify(adminbotnumber))
                   reply('Success Menambahkan Admin DARK BOT!')
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				default:
                  ...
                  if (budy == 'cekprefix') {
                  costum(`*DARK BOT USING PREFIX [ ${prefix} ]*`, text, tescuk, cr)
                  }
